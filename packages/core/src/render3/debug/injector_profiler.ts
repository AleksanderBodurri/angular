...
export function emitProviderConfiguredEvent(
  ...
  // Injection tokens may define their own provider which gets attached to the token itself as
  // `ɵprov`. In this case, we want to emit the provider that is attached to the token, not the
  // token itself.
