...
import { ClassProvider, Component, destroyPlatform, Injectable, InjectFlags, Injector, NgModule } from '@angular/core/src/core';
...
import { getDependenciesFromInstantiation, getInjectorProviders } from '@angular/core/src/render3/util/discovery_utils';
...
describe('injector profiler', () => {
  ...
  describe('getDependenciesFromInstantiation', () => {
    beforeEach(() => setupFrameworkInjectorProfiler());
    afterAll(() => setInjectorProfiler(null));

    it('should be able to determine instance dependencies given an injector',
      withBody('<my-comp></my-comp>', async () => {
        destroyPlatform();
        ...
        const { instance, dependencies } = getDependenciesFromInstantiation(
          applicationRef.components[0].injector,
          applicationRef.components[0].componentType
        )!;
        const standaloneInjector = applicationRef.components[0].injector.get(EnvironmentInjector);
        ...
        expect(myServiceDependency.flags).toBe(0);
        expect(myServiceDependency.providedIn).toBe(standaloneInjector);
        
        const {instance: myServiceInstance, dependencies: myServiceDependencies } = getDependenciesFromInstantiation(
          myServiceDependency.providedIn!,
          myServiceDependency.token!
        )!;
        ...
        expect(myServiceBDependency.flags).toBe(0);
        expect(myServiceBDependency.providedIn).toBe(standaloneInjector);

        const { instance: myServiceBInstance, dependencies: myServiceBDependencies } = getDependenciesFromInstantiation(
          myServiceBDependency.providedIn!,
          myServiceBDependency.token!
        )!;
        ...
        expect(routerDependency.flags).toBe(0);
        expect(routerDependency.providedIn).toBe(applicationRef.injector);
